/*
    PULSE — PR-EXP-C03C1
    Extend the restored Flow-facing export procedure
    warroom.usp_ExportProjectPunchesExtended_Pivoted
    with an OPTIONAL exact Punch Review Queue scope.

    PURPOSE
    -------
    The active Power Automate flow Warroom_ExportPunchesToExcel_Codex calls
    warroom.usp_ExportProjectPunchesExtended_Pivoted. The procedure has now
    been restored in the database. This incremental deployment adds only the
    optional Review Queue contract required by Punch Review.

    LEGACY SAFETY
    -------------
    - Existing callers remain valid because @WorkItemIdsJson is an OPTIONAL
      trailing parameter.
    - @WorkItemIdsJson IS NULL preserves the existing filtered export path.
    - A supplied Review Queue must resolve with exact cardinality.
    - Partial exports are forbidden.
    - The script is anchor-guarded and stops before ALTER PROCEDURE if the live
      definition does not match the expected baseline.
    - No Punch, Comment or Custom Field business data is modified.

    PROJECT IDENTIFIER RULE
    -----------------------
    @ProjectId is the INTERNAL project id used by dbo.wap_PunchPaged.ProjectId.
    For the PULSE project shown to users as 70200, the validated internal id is
    4049.
*/
...
IF @ObjectId IS NULL
    THROW 52200, 'PR-EXP-C03C1: restored pivoted export procedure was not found.', 1;
...
IF @Definition IS NULL
    THROW 52201, 'PR-EXP-C03C1: pivoted export procedure definition could not be read.', 1;

-- Normalise line endings only in the deployment working copy.
...
(
    VARCHAR(64),
    HASHBYTES('SHA2_256', CONVERT(VARBINARY(MAX), @OriginalDefinition)),
    2
);
...
BEGIN
    IF CHARINDEX(N'THROW 52216', @Definition) = 0
        THROW 52218, 'PR-EXP-C03C1: @WorkItemIdsJson exists but the exact-scope guard is missing. Manual review required.', 1;

    SELECT
        ProcedureName = @ProcedureName,
        Deployment = CAST(N'PR-EXP-C03C1_ALREADY_PRESENT' AS NVARCHAR(40)),
...
IF CHARINDEX(@Anchor, @Definition) = 0
    THROW 52202, 'PR-EXP-C03C1: pivoted procedure signature anchor was not found. Deployment stopped.', 1;
...
SET @Replacement = @Anchor + N'
    ...
    ---------------------------------------------------------------------
    -- PR-EXP-C03C1) Optional exact Punch Review Queue scope
    ---------------------------------------------------------------------
    ...
    BEGIN
        IF @TemplateId IS NULL
            THROW 52210, ''REVIEW_QUEUE export requires TemplateId.'', 1;

        IF ISJSON(@WorkItemIdsJson) <> 1
           OR LEFT(LTRIM(@WorkItemIdsJson), 1) <> N''[''
            THROW 52211, ''WorkItemIdsJson must be a valid JSON array.'', 1;
        ...
        INSERT INTO #RequestedReviewStage (SourceOrdinal, WorkItemId)
        SELECT
        ...
        IF NOT EXISTS (SELECT 1 FROM #RequestedReviewStage)
            THROW 52212, ''WorkItemIdsJson cannot be an empty array.'', 1;
        ...
        (
            ...
            THROW 52213, ''Every WorkItemId must be a positive integer.'', 1;
        ...
        (
            ...
            THROW 52214, ''WorkItemIdsJson contains duplicate WorkItemId values.'', 1;
...
IF CHARINDEX(@Anchor, @Definition) = 0
    THROW 52203, 'PR-EXP-C03C1: pivoted procedure initialisation anchor was not found. Deployment stopped.', 1;

SET @Definition = REPLACE(@Definition, @Anchor, @Replacement);

/* ================================================================
   PATCH 3 — constrain PunchBase by exact ids when supplied
   ================================================================ */
SET @Anchor =
    N'        WHERE' + CHAR(10) +
...
IF CHARINDEX(@Anchor, @Definition) = 0
    THROW 52204, 'PR-EXP-C03C1: pivoted PunchBase WHERE anchor was not found. Deployment stopped.', 1;

SET @Definition = REPLACE(@Definition, @Anchor, @Replacement);

/* ================================================================
   PATCH 4 — exact cardinality invariant after all existing filters
   ================================================================ */
SET @Anchor =
    N'    ---------------------------------------------------------------------' + CHAR(10) +
...
SET @Replacement = N'
    ---------------------------------------------------------------------
    -- PR-EXP-C03C1) Exact Review Queue cardinality invariant
    ---------------------------------------------------------------------
    IF @WorkItemIdsJson IS NOT NULL
    BEGIN
        ...
        BEGIN
            ...
            THROW 52216, @ReviewScopeError, 1;
        END;
    END;
...
IF CHARINDEX(@Anchor, @Definition) = 0
    THROW 52205, 'PR-EXP-C03C1: pivoted safety-limit anchor was not found. Deployment stopped.', 1;

SET @Definition = REPLACE(@Definition, @Anchor, @Replacement);

/* ================================================================
   Generated-definition guards
   ================================================================ */
IF @Definition = @OriginalDefinition
    THROW 52206, 'PR-EXP-C03C1: no changes were generated. Deployment stopped.', 1;

IF CHARINDEX(N'@WorkItemIdsJson     NVARCHAR(MAX) = NULL', @Definition) = 0
    THROW 52207, 'PR-EXP-C03C1: optional parameter was not generated.', 1;

IF CHARINDEX(N'#RequestedReviewScope', @Definition) = 0
    THROW 52208, 'PR-EXP-C03C1: Review Queue staging was not generated.', 1;

IF CHARINDEX(N'THROW 52216', @Definition) = 0
    THROW 52209, 'PR-EXP-C03C1: exact cardinality guard was not generated.', 1;

/* ================================================================
   Convert live CREATE / CREATE OR ALTER into ALTER PROCEDURE
   ================================================================ */
...
IF @CreatePos = 0
   ...
   OR @CreatePos > @ProcedurePos
    THROW 52219, 'PR-EXP-C03C1: CREATE PROCEDURE prefix could not be located safely. Deployment stopped.', 1;
...
(
    ...
    THROW 52217, 'PR-EXP-C03C1: deployment completed but @WorkItemIdsJson was not found.', 1;
...
IF CHARINDEX(N'THROW 52216', @DeployedDefinition) = 0
    THROW 52221, 'PR-EXP-C03C1: deployment completed but exact cardinality guard was not found.', 1;

SET @DeployedHash = CONVERT
(
    VARCHAR(64),
    HASHBYTES('SHA2_256', CONVERT(VARBINARY(MAX), @DeployedDefinition)),
    2
);

SELECT
    ProcedureName = @ProcedureName,
    Deployment = CAST(N'PR-EXP-C03C1' AS NVARCHAR(30)),
    SourceDefinitionHash = @SourceHash,
